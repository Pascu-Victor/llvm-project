//===--- WOS.cpp - WOS ToolChain Implementations --------*- C++ -*-===//
//
...
void wos::Linker::ConstructJob(Compilation &C, const JobAction &JA,
                               const InputInfo &Output,
                               const InputInfoList &Inputs, const ArgList &Args,
                               const char *LinkingOutput) const {
  const auto &ToolChain = static_cast<const WOS &>(getToolChain());
...
void wos::StaticLibTool::ConstructJob(Compilation &C, const JobAction &JA,
                                      const InputInfo &Output,
...
WOS::WOS(const Driver &D, const llvm::Triple &Triple, const ArgList &Args)
...
std::string WOS::ComputeEffectiveClangTriple(const ArgList &Args,
                                             types::ID InputType) const {
  llvm::Triple Triple(ComputeLLVMTriple(Args, InputType));
  return Triple.str();
}

Tool *WOS::buildLinker() const { return new tools::wos::Linker(*this); }

Tool *WOS::buildStaticLibTool() const {
  return new tools::wos::StaticLibTool(*this);
}
...
void WOS::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
...
void WOS::AddCXXStdlibLibArgs(const ArgList &Args,
...
SanitizerMask WOS::getDefaultSanitizers() const {
  SanitizerMask Res;
